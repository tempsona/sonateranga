 ...
 * @author sidibe0091
 */
public class SVC_M_CFNREPLY extends Charge {

    public SVC_M_CFNREPLY(String code, TYPE_CHARGE type, TYPE_EVENT declencheur, boolean aProvisionner) {
        super(code, type, declencheur, aProvisionner);
        setDescription("Renvoi si pas de réponse");
        setEtat(ETAT.ACTIF);
        setObligatoire(true);
    ...
    public List<Conso> getConso(Contrat contrat) {
        ...
        try {
            ...
            Logger.getLogger(SVC_M_CFNREPLY.class.getName()).log(Level.SEVERE, null, ex);
        }
        return lc;
    }
}
