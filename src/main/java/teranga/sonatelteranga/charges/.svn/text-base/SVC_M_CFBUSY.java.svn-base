 ...
 * @author sidibe0091
 */
public class SVC_M_CFBUSY extends Charge {
    ...
    public List<Conso> getConso(Contrat contrat) {
        ...
        try {
            ...
            Logger.getLogger(SVC_M_CFBUSY.class.getName()).log(Level.SEVERE, null, ex);
        }
        return lc;
    }

    public SVC_M_CFBUSY(String code, TYPE_CHARGE type, TYPE_EVENT declencheur, boolean aProvisionner) {
        super(code, type, declencheur, aProvisionner);
        setDescription("Renvoi si occupé");
        setEtat(ETAT.ACTIF);
        setObligatoire(true);
