 ...
 * @author sidibe0091
 */
public class SVC_M_CLIP extends Charge {

    public SVC_M_CLIP(String code, TYPE_CHARGE type, TYPE_EVENT declencheur, boolean aProvisionner) {
        super(code, type, declencheur, aProvisionner);
        setDescription("CLIP - Présentation du numéro");             
        setObligatoire(true);
        setEtat(ETAT.ACTIF);
    ...
    public List<Conso> getConso(Contrat contrat) {
        ...
        try {
            ...
            Logger.getLogger(SVC_M_CLIP.class.getName()).log(Level.SEVERE, null, ex);
        }
        return lc;
    }
}
